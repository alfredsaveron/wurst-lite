	...
	ShouldDrawSideListener, RenderBlockEntityListener
{
	...
	private void triggerGradualReload()
	{
		if(MC.player == null || MC.level == null)
			return;
		
		final int pSectionX = MC.player.blockPosition().getX() >> 4;
		final int pSectionY = MC.player.blockPosition().getY() >> 4;
		final int pSectionZ = MC.player.blockPosition().getZ() >> 4;
		final int viewDistance = MC.options.getEffectiveRenderDistance();
		final int minSectionY = MC.level.getMinSection();
		final int maxSectionY = MC.level.getMaxSection();
		...
		WURST.getEventManager().add(UpdateListener.class, new UpdateListener()
		{
			...
			public void onUpdate()
			{
				if(MC.level == null || MC.player == null
					|| stage > viewDistance + 2)
				{
					WURST.getEventManager().remove(UpdateListener.class, this);
					return;
				}
				
				for(int dx = -stage; dx <= stage; dx++)
					for(int dz = -stage; dz <= stage; dz++)
						for(int dy = -stage; dy <= stage; dy++)
						{
							if(Math.max(Math.abs(dx),
								Math.max(Math.abs(dy), Math.abs(dz))) != stage)
								continue;
