...
public final class ClickGui
{
	...
	public void init()
	{
		...
		sortedWindows.sort((a, b) -> b.countChildren() - a.countChildren());
		windows.addAll(sortedWindows);
	...
	public void handleMouseClick(int mouseX, int mouseY, int mouseButton)
	{
		...
		for(Popup popup : popups)
			if(popup.getOwner().getParent() != null
				&& popup.getOwner().getParent().isClosing())
				popup.close();
	...
	public void renderTooltip(GuiGraphics context, int mouseX, int mouseY)
	{
		...
		for(int i = 0; i < lines.length; i++)
			context.drawString(tr, modern(lines[i]), xt1 + 2,
		...
		int mouseY, float partialTicks)
	{
		...
		int windowBgColor = 0xFF0A0A0A;
		int outlineColor = 0xFFA273A6;
		...
		if(!window.isMinimized())
		{
			...
			if(window.isScrollingEnabled())
			{
				...
				// scrollbar
				int scrollbarColor = hovering ? 0xFFD8A8DC : 0xFFA273A6;
		...
		int titleBgColor = 0xFFA273A6;
		context.fill(x1, y1, x2, y3, titleBgColor);
		
		Font tr = MC.font;
		String titleText = window.getTitle();
