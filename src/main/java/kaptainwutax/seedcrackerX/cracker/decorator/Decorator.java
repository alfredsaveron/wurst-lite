...
public abstract class Decorator<C extends Decorator.Config, D extends Decorator.Data<?>>
	extends Feature<C, D>
{
	...
	public boolean canStart(D data, long structureSeed, ChunkRand rand)
	{
		rand.setDecoratorSeed(structureSeed, data.chunkX << 4, data.chunkZ << 4,
			this.getIndex(data.biome), this.getStep(data.biome),
			this.getVersion());
		return true;
	}
	...
	{
		long l = rand.setDecorationSeed(worldSeed, data.chunkX << 4,
			data.chunkZ << 4);
		rand.setFeatureSeed(l, this.getIndex(data.biome),
			this.getStep(data.biome));
		return true;
	}
	...
	public boolean canSpawn(int chunkX, int chunkZ, BiomeSource source)
	{
		...
		{
			return this.isValidBiome(
				source.getBiome((chunkX << 4) + 8, 0, (chunkZ << 4) + 8));
		}
		
		return this.isValidBiome(source.getBiomeForNoiseGen((chunkX << 2) + 2,
			0, (chunkZ << 2) + 2));
	}
