...
public class Dungeon extends Decorator<Decorator.Config, Dungeon.Data>
{
	...
	public static final VersionMap<Config> CONFIGS =
		new VersionMap<Decorator.Config>()
			.add(MCVersion.v1_13, new Decorator.Config(2, 3))
			.add(MCVersion.v1_16,
				new Decorator.Config(3, 2).add(3, 3, Biomes.DESERT,
	...
	@Override
	public boolean canStart(Dungeon.Data data, long structureSeed,
		ChunkRand rand)
	{
		...
		for(int i = 0; i < 8; i++)
		{
			...
			}else
			{
				x = rand.nextInt(16);
				z = rand.nextInt(16);
				y = rand.nextInt(data.heightContext.getHeight())
					+ data.heightContext.getBottomY();
			}
	...
	@Override
	public boolean canStart(Dungeon.Data data, long worldSeed,
		WorldgenRandom rand)
	{
	...
	{
		return biome != Biomes.NETHER_WASTES && biome != Biomes.SOUL_SAND_VALLEY
			&& biome != Biomes.WARPED_FOREST && biome != Biomes.CRIMSON_FOREST
			&& biome != Biomes.BASALT_DELTAS && biome != Biomes.END_MIDLANDS
			&& biome != Biomes.END_HIGHLANDS && biome != Biomes.END_BARRENS
			&& biome != Biomes.SMALL_END_ISLANDS && biome != Biomes.THE_VOID
			&& biome == Biomes.THE_END;
	}
	
	public Dungeon.Data at(int blockX, int blockY, int blockZ, Vec3i size,
		int[] floorCalls, Biome biome, HeightContext heightContext)
	{
		return new Dungeon.Data(this, blockX, blockY, blockZ, size, floorCalls,
			biome, heightContext);
	}
	...
	public static class Data extends Decorator.Data<Dungeon>
	{
		...
		public Data(Dungeon feature, int blockX, int blockY, int blockZ,
			Vec3i size, int[] floorCalls, Biome biome,
		...
		{
			return this.bitsCount >= MIN_FLOOR_BITS
				&& this.bitsCount <= MAX_FLOOR_BITS;
		}
		...
		public void onDataAdded(DataStorage dataStorage)
		{
			...
			if(dataStorage.getTimeMachine().shouldTerminate)
				return;
			if(dataStorage.getTimeMachine().worldSeeds.size() == 1)
				return;
			if(dataStorage.getTimeMachine().structureSeeds.size() == 1)
				return;
			if(this.feature.getVersion().isNewerThan(MCVersion.v1_17_1))
				return;
			if(this.floorCalls == null || !this.usesFloor())
				return;
			...
			if(debug)
			{
				...
				Log.printDungeonInfo(this.blockX + ", " + this.blockY + ", "
					+ this.blockZ + ", \"" + floorString + "\"");
			...
			}else
			{
				device.add(JavaCalls.nextInt(16).equalTo(this.offsetX));
				device.add(JavaCalls.nextInt(16).equalTo(this.offsetZ));
				device.add(JavaCalls.nextInt(heightContext.getHeight())
					.equalTo(heightContext.getDistanceToBottom(this.blockY)));
			}
			device.skip(2);
			...
			Set<Long> decoratorSeeds =
				device.reverse().parallel().boxed().collect(Collectors.toSet());
			...
			if(this.feature.getVersion().isOlderThan(MCVersion.v1_13))
			{
				for(long decoratorSeed : decoratorSeeds)
				{
					...
					for(int i = 0; i < 200; i++)
					{
						for(long structureSeed : ChunkRandomReverser
							.reversePopulationSeed(
								decoratorSeed ^ LCG.JAVA.multiplier,
								blockX >> 4, blockZ >> 4, MCVersion.v1_12_2))
						{
							if(!dataStorage.getTimeMachine().structureSeeds
								.add(structureSeed))
							{
						...
						decoratorSeed =
							LCG.JAVA.combine(-1).nextSeed(decoratorSeed);
					}
				}
				if(result.isEmpty())
				{
							...
							.reversePopulationSeed(
								(decoratorSeed ^ LCG.JAVA.multiplier)
									- this.feature.getConfig()
										.getSalt(this.biome),
								this.chunkX << 4, this.chunkZ << 4,
								this.feature.getVersion())
							.forEach(structureSeed -> {
								Log.printSeed("foundStructureSeed",
									structureSeed);
								if(!dataStorage.getTimeMachine().structureSeeds
									.add(structureSeed))
								{
