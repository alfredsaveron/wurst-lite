...
public class DataStorage
{
	...
	public static final Comparator<Entry<Feature.Data<?>>> SEED_DATA_COMPARATOR =
		(s1, s2) -> {
			...
			double diff = getBits(s2.data.feature, false)
				- getBits(s1.data.feature, false);
			return diff == 0 ? 1 : (int)Math.signum(diff);
		};
	public ScheduledSet<Entry<Feature.Data<?>>> baseSeedData =
		new ScheduledSet<>(SEED_DATA_COMPARATOR);
	...
	protected Set<Consumer<DataStorage>> scheduledData =
		ConcurrentHashMap.newKeySet();
	protected PillarData pillarData = null;
	protected ScheduledSet<Entry<BiomeData>> biomeSeedData =
		new ScheduledSet<>(null);
	...
	public static double getBits(Feature<?, ?> feature, boolean decorators18)
	{
		...
		if(!decorators18 && feature instanceof Decorator
			&& feature.getVersion().isNewerThan(MCVersion.v1_17_1))
			return 0;
		if(feature instanceof BuriedTreasure)
			return Math.log(100) / Math.log(2);
		if(feature instanceof DesertWell)
			return Math.log(1000 * 16 * 16) / Math.log(2);
		if(feature instanceof Dungeon)
			return Math.log(256 * 16 * 16 * 0.125D) / Math.log(2);
		if(feature instanceof DeepDungeon)
			return Math.log(58 * 16 * 16 * 0.25D) / Math.log(2);
		if(feature instanceof EmeraldOre)
			return Math.log(28 * 16 * 16 * 0.5D) / Math.log(2);
		
		throw new UnsupportedOperationException(
			"go do implement bits count for " + feature.getName()
				+ " you fool");
	}
	...
	public synchronized boolean addPillarData(PillarData data,
	...
	public synchronized boolean addBaseData(Feature.Data<?> data,
		DataAddedEvent event)
	{
	...
	public synchronized boolean addBiomeData(BiomeData data,
		DataAddedEvent event)
	{
	...
	public synchronized boolean addHashedSeedData(HashedSeedData data,
		DataAddedEvent event)
	{
		if(this.hashedSeedData == null
			|| this.hashedSeedData.getHashedSeed() != data.getHashedSeed())
		{
	...
	public double getLiftingBits()
	{
		...
		for(Entry<Feature.Data<?>> e : this.baseSeedData)
		{
			...
			{
				bits += Math.log(structure.getOffset() * structure.getOffset())
					/ Math.log(2);
			}else if(e.data.feature instanceof Shipwreck shipwreck)
			{
				bits += Math.log(shipwreck.getOffset() * shipwreck.getOffset())
					/ Math.log(2);
			}
		}
		return bits;
	...
	public static class Entry<T>
	{
		...
		public boolean equals(Object o)
		{
			...
			if(this.data instanceof Feature.Data<?> d1
				&& entry.data instanceof Feature.Data<?> d2)
			{
				return d1.feature == d2.feature && d1.chunkX == d2.chunkX
					&& d1.chunkZ == d2.chunkZ;
			}else if(this.data instanceof BiomeData
				&& entry.data instanceof BiomeData)
			{
		...
		public int hashCode()
		{
			...
			{
				return ((Feature.Data<?>)this.data).chunkX * 31
					+ ((Feature.Data<?>)this.data).chunkZ;
			}else if(this.data instanceof BiomeData)
			{
