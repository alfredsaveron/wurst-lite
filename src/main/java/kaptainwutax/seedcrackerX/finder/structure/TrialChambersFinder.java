...
public class TrialChambersFinder extends Finder
{
	...
	public static void buildOminousVault(JigsawFinder finder, int x, int y,
		int z)
	{
		...
		var redCandle4 = Blocks.RED_CANDLE.defaultBlockState()
			.setValue(CandleBlock.CANDLES, 4).setValue(CandleBlock.LIT, true);
		var waxedOxidizedCopperGrate =
			Blocks.WAXED_OXIDIZED_COPPER_GRATE.defaultBlockState();
		
		finder.fillWithOutline(x, y, z, x + 2, y, z + 1,
			waxedOxidizedCopperGrate, waxedOxidizedCopperGrate, false);
		finder.addBlock(redCandle3, x, y + 1, z);
		finder.addBlock(redCandle4, x + 2, y + 1, z);
		finder.fillWithOutline(x, y + 1, z + 1, x + 2, y + 3, z + 1,
			polishedTuff, polishedTuff, false);
		finder.fillWithOutline(x, y + 2, z + 1, x + 2, y + 2, z + 1,
			polishedTuffSlabTop, polishedTuffSlabTop, false);
		finder.addBlock(ominousVault, x + 1, y + 1, z + 1);
		finder.addBlock(air, x + 1, y + 2, z + 1);
		finder.addBlock(waxedCopperBulbOn, x + 1, y + 3, z + 1);
		
		finder.fillWithOutline(x, y, z + 2, x + 2, y, z + 2, polishedTuff,
			polishedTuff, false);
		finder.fillWithOutline(x, y + 1, z + 2, x + 2, y + 3, z + 2, tuffBricks,
			tuffBricks, false);
		finder.addBlock(waxedCopperBulbOn, x + 1, y + 1, z + 2);
		finder.fillWithOutline(x, y + 2, z + 2, x + 2, y + 2, z + 2,
			redGlazedTerracotta, redGlazedTerracotta, false);
	...
	public static void buildEnd1(JigsawFinder finder)
	{
		var air = Blocks.AIR.defaultBlockState();
		var tuffBricks = Blocks.TUFF_BRICKS.defaultBlockState();
		var waxedOxidizedCutCopper =
			Blocks.WAXED_OXIDIZED_CUT_COPPER.defaultBlockState();
		var waxedCopperBulbOn = Blocks.WAXED_COPPER_BULB.defaultBlockState()
			.setValue(CopperBulbBlock.LIT, true);
		var waxedCopperBlock = Blocks.WAXED_COPPER_BLOCK.defaultBlockState();
		var chiseledTuff = Blocks.CHISELED_TUFF.defaultBlockState();
		var chiseledTuffBricks =
			Blocks.CHISELED_TUFF_BRICKS.defaultBlockState();
		var polishedTuff = Blocks.POLISHED_TUFF.defaultBlockState();
		
		// Sides
		finder.fillWithOutline(0, 0, 0, 0, 18, 18, tuffBricks, tuffBricks,
			false);
		finder.fillWithOutline(18, 0, 0, 18, 18, 18, tuffBricks, tuffBricks,
			false);
		
		// Top
		finder.fillWithOutline(0, 19, 0, 18, 19, 18, tuffBricks, tuffBricks,
			false);
		
		// Bottom
		finder.fillWithOutline(0, 0, 0, 18, 1, 18, tuffBricks, tuffBricks,
			false);
		
		// Wall
		// finder.fillWithOutline(6, 4, 11, 12, 18, 12, tuffBricks, tuffBricks,
		// false);
		finder.fillWithOutline(6, 6, 11, 12, 18, 12, tuffBricks, tuffBricks,
			false);
		
		// Base
		finder.fillWithOutline(6, 3, 11, 12, 3, 12, waxedOxidizedCutCopper,
			waxedOxidizedCutCopper, false);
		
		// Copper Bulb
		// finder.addBlock(waxedCopperBulbOn, 9, 4, 12);
		
		// Copper rings
		finder.fillWithOutline(6, 10, 11, 12, 10, 12, waxedCopperBlock,
			waxedCopperBlock, false);
		finder.fillWithOutline(6, 15, 11, 12, 15, 12, waxedCopperBlock,
			waxedCopperBlock, false);
		
		// Chiseled tuff line
		finder.fillWithOutline(6, 5, 12, 12, 5, 12, chiseledTuff, chiseledTuff,
			false);
		finder.fillWithOutline(6, 5, 11, 7, 5, 12, waxedCopperBlock,
			waxedCopperBlock, false);
		finder.fillWithOutline(11, 5, 11, 12, 5, 11, waxedCopperBlock,
			waxedCopperBlock, false);
		finder.fillWithOutline(7, 5, 12, 12, 5, 12, chiseledTuffBricks,
			chiseledTuffBricks, false);
		finder.addBlock(chiseledTuff, 8, 5, 12);
		finder.addBlock(chiseledTuff, 10, 5, 12);
		
		// Chiseled tuff brick rings
		finder.fillWithOutline(6, 7, 11, 12, 7, 12, chiseledTuffBricks,
			chiseledTuffBricks, false);
		finder.fillWithOutline(6, 12, 11, 12, 12, 12, chiseledTuffBricks,
			chiseledTuffBricks, false);
		finder.fillWithOutline(6, 17, 11, 12, 17, 12, chiseledTuffBricks,
			chiseledTuffBricks, false);
		
		// Carve
		finder.fillWithOutline(8, 3, 11, 10, 17, 11, air, air, false);
		finder.fillWithOutline(8, 18, 11, 10, 18, 12, polishedTuff,
			polishedTuff, false);
	...
	public static void buildEnd2(JigsawFinder finder)
	{
		...
		// Bottom
		finder.fillWithOutline(0, 0, 0, 18, 0, 18, tuffBricks, tuffBricks,
			false);
		finder.fillWithOutline(8, 0, 13, 10, 0, 17, waxedOxidizedCopper,
			waxedOxidizedCopper, false);
		...
		// Grates
		finder.fillWithOutline(8, 3, 11, 10, 5, 11, waxedCopperGrate,
			waxedCopperGrate, false);
		finder.fillWithOutline(7, 3, 13, 7, 5, 16, waxedCopperGrate,
			waxedCopperGrate, false);
		finder.fillWithOutline(11, 3, 13, 11, 5, 16, waxedCopperGrate,
			waxedCopperGrate, false);
	...
	{
		Biome biome = this.world.getNoiseBiome((this.chunkPos.x << 2) + 2, 64,
			(this.chunkPos.z << 2) + 2).value();
		// TODO: replace with following once deep dark is implemented
		// if (!Features.TRIAL_CHAMBERS.isValidBiome(BiomeFixer.swap(biome)))
		// return new ArrayList<>();
		if(net.minecraft.resources.ResourceLocation
			.withDefaultNamespace("deep_dark")
			.equals(Minecraft.getInstance().level.registryAccess()
		...
		result.forEach((pieceFinder, positions) -> {
			...
			positions.forEach(pos -> {
				RegionStructure.Data<?> data = Features.TRIAL_CHAMBERS
					.at(this.chunkPos.x, this.chunkPos.z);
				
				if(SeedCracker.get().getDataStorage().addBaseData(data,
					DataAddedEvent.POKE_STRUCTURES))
				{
					this.cuboids.add(new Cuboid(pos, pieceFinder.getLayout(),
						ARGB.color(170, 84, 3)));
					this.cuboids.add(new Cuboid(
						chunkPos.getWorldPosition().offset(0, pos.getY(), 0),
						ARGB.color(170, 84, 3)));
				}
			});
		});
