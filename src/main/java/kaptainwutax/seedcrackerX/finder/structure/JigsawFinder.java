...
{
	public JigsawFinder(Level world, ChunkPos chunkPos, Direction facing,
		Vec3i size)
	{
		super(world, chunkPos, facing, size);
	}
	
	public static Map<Direction, List<BlockPos>> getSearchPositions(
		int xRotation, int zRotation, int xOffset, int zOffset, Vec3i size)
	{
		...
		for(Direction direction : Direction.Plane.HORIZONTAL)
		{
			...
			{
				int startIndex = heightContext.getHeight() * x * 16
					+ heightContext.getHeight() * z;
				for(int y = 0; y < heightContext.getHeight(); y++)
				{
					positions.get(direction)
						.add(CHUNK_POSITIONS.get(y + startIndex));
				}
			}else
			{
				for(int y = heightContext.getBottomY(); y < heightContext
					.getTopY(); y++)
				{
