...
public class Database
{
	...
	private static final String DATABASE_POST_URL =
		"https://script.google.com/macros/s/AKfycbye87L-fEYq2EkgczvhKb_kGecp5wL1oX95vg45TRSwNvpv7K-53zoInGTeI1FZ0kv7DA/exec";
	private static final String DATABASE_URL =
		"https://docs.google.com/spreadsheets/d/1tuQiE-0leW88em9OHbZnH-RFNhVqgoHhIt9WQbeqqWw/export?format=csv&gid=0"; // script
																														// link
																														// is
																														// not
																														// updated,
	...
	public static Component joinFakeServerForAuth()
	{
		try
		{
			...
			client.getMinecraftSessionService().joinServer(
				...
				client.getUser().getAccessToken(), "seedcrackerx");
		}catch(AuthenticationUnavailableException authenticationUnavailableException)
		{
			return Component.translatable("disconnect.loginFailedInfo",
		...
		{
			return Component.translatable("disconnect.loginFailedInfo",
				authenticationUnavailableException.getMessage());
		}
		return null;
	}
	...
	public static void handleDatabaseCall(Long seed)
	{
		...
		data.put("serverIp", client.getConnection().getConnection()
			.getRemoteAddress().toString());
		data.put("dimension",
			Finder.inferDimension(client.level.dimensionType()));
		data.put("seed", seed + "L"); // javascript backend likes floating
										// point. so we need to convert it to a
										// string
		...
		HttpRequest request = HttpRequest
			.newBuilder(URI.create(DATABASE_POST_URL)).timeout(TIMEOUT)
			.POST(HttpRequest.BodyPublishers
				.ofString(HttpAuthenticationService.buildQuery(data)))
	...
	{
		HttpRequest request =
			HttpRequest.newBuilder(URI.create(DATABASE_URL)).timeout(TIMEOUT)
				.GET().setHeader("User-Agent", SEEDCRACKERX_USER_AGENT).build();
		HTTP_CLIENT.sendAsync(request, HttpResponse.BodyHandlers.ofString())
			.thenApply(HttpResponse::body).thenAccept(Database::parseCsv);
	}
		...
		Arrays.stream(csv.split("\n")).skip(1).forEach(row -> {
			try
			{
				...
				long seed = Long.parseLong(
					seedString.substring(0, seedString.length() - 1));
