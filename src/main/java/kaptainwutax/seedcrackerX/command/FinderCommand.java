...
public class FinderCommand extends ClientCommand
{
	...
	public void build(LiteralArgumentBuilder<FabricClientCommandSource> builder)
	{
		...
		{
			builder.then(literal("type").then(literal(finderType.toString())
				.then(literal("ON").executes(
					context -> this.setFinderType(finderType, true, true)))
				.then(literal("OFF").executes(
					context -> this.setFinderType(finderType, false, true)))
		...
		{
			builder.then(
				literal("category").then(literal(finderCategory.toString())
					.then(literal("ON").executes(context -> this
						.setFinderCategory(finderCategory, true)))
					.then(literal("OFF").executes(context -> this
						.setFinderCategory(finderCategory, false)))
					.executes(
						context -> this.printFinderCategory(finderCategory))));
		}
	...
	{
		Finder.Type.getForCategory(finderCategory)
			.forEach(this::printFinderType);
		return 0;
	}
	...
	{
		sendFeedback(
			Log.translate("finder.isFinder")
				.formatted(Log.translate(finderType.nameKey)) + " ["
				+ String.valueOf(finderType.enabled.get()).toUpperCase() + "].",
	...
	{
		Finder.Type.getForCategory(finderCategory)
			.forEach(finderType -> this.setFinderType(finderType, flag, false));
		Config.save();
		return 0;
	}
		...
		boolean save)
	{
		...
		if(save)
			Config.save();
		sendFeedback(
			Log.translate("finder.setFinder")
				.formatted(Log.translate(finderType.nameKey)) + " ["
				+ String.valueOf(flag).toUpperCase() + "].",
			ChatFormatting.AQUA, false);
		return 0;
	}
