...
public class CrackerCommand extends ClientCommand
{
	...
	{
		String action =
			Log.translate(flag ? "cracker.enabled" : "cracker.disabled");
		if(success)
		{
			sendFeedback(Log.translate("cracker.successfully") + action,
				ChatFormatting.GREEN, false);
		}else
		{
			sendFeedback(Log.translate("cracker.already") + action,
