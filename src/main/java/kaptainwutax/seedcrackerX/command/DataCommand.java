...
public class DataCommand extends ClientCommand
{
	...
	public void build(LiteralArgumentBuilder<FabricClientCommandSource> builder)
	{
		...
		builder.then(literal("bits").executes(this::printBits));
		
		builder.then(literal("restore").executes(this::restoreData));
	}
	...
	public int clear(CommandContext<FabricClientCommandSource> context)
	{
		...
		sendFeedback(Language.getInstance().getOrDefault("data.clearData"),
	...
	private int printBits(CommandContext<FabricClientCommandSource> context)
	{
		...
		String message =
			Language.getInstance().getOrDefault("data.collectedBits")
				.formatted((int)s.getBaseBits(), (int)s.getWantedBits());
		String message2 =
			Language.getInstance().getOrDefault("data.collectedLiftingBits")
				.formatted((int)s.getLiftingBits(), 40);
